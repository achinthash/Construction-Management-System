...
export default function TaskAssignedUsers() {
  ...
  const [isUserAvailability, setIsUserAvailability] = useState(false);
  ...
  const setSuccessMessage = useRef(null);
    ...
    useEffect(()=>{
      ...
      const userLogs = async() =>{
        try{
          const response = await axios.get(`http://localhost:8000/api/user-logs/${taskId}`,{
  ...
  <div className=''>
        ...
        <div className="flex absolute inset-0 items-center justify-center z-10 bg-black bg-opacity-75 " > 
          <div className='bg-white border border-violet-950 rounded-lg max-w-[80%] min-w-[70%] '>
            ...
            <div  className="bg-[#ddd6fe] rounded p-2 mr-2 ml-2 mt-2 grid grid-cols-2 mb-2">
              ...
              <button  onClick={()=>setIsUserAvailability(false)} type='button'  className='ml-auto items-center col-span-1'>
    ...
    <div className="bg-[#cec8ecb7]  dark:bg-gray-900  rounded p-1.5 mt-1 mr-1 flex justify-between items-center ">
      <h1 className="text-left sm:text-xl font-bold text p-1 text-[#5c3c8f] dark:text-white ">Task Assignee</h1>
        {
          ( userInfo?.role === 'admin' ||  userInfo?.role === 'contractor' ) && (
            <button onClick={()=>setIsUserAvailability(true)}  className="py-2 px-3 ms-2 text-sm font-medium text-white bg-[#6d28d9] dark:bg-gray-500  hover:dark:bg-gray-400 rounded-lg   hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 "> <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#FFFFFF"><path d="M440-440H200v-80h240v-240h80v240h240v80H520v240h-80v-240Z"/></svg> </button>
          )
        }
    </div>
...

      {
        userLogs.map((user) =>(
          ...
          <div key={user.user_id}  >
            ...
            <div  onClick={()=>userlogExtra(user.user_id)} className="p-4 bg-white shadow-md rounded-lg border border-gray-200 grid grid-cols-4 gap-2 w-full cursor-pointer hover:bg-violet-100 mt-2">
              <div className="flex items-center gap-4 col-span-2">
                <img src={`http://localhost:8000/storage/${user.user_profile_picture}`}   alt={user.user_name}  className="w-12 h-12 rounded-full object-cover border border-gray-300"/>
                <div>
                  <h2 className="text-lg font-semibold text-gray-800">{user.user_name}</h2>
                  <p className="text-sm text-gray-600">{user.user_role}</p>
                  <p className="text-sm text-gray-600">{user.user_position}</p>
                </div>
              </div>

              <div className='col-span-1 items-center '> 
                <p className={`text-sm ${user.user_status === 'active' ? 'text-green-500' : ' text-red-500 '}`}>Status: {user.user_status}</p>
              </div>

              <div className="col-span-1 flex flex-col  items-start space-y-2 ">
                <p className="text-sm text-gray-600">Progress: <span className="font-medium text-blue-600">{user.progress_percentage}%</span></p>
                {/* <p className="text-sm text-gray-600">Total Cost: <span className="font-medium text-gray-800">${user.total_cost.toFixed(2)}</span></p> */}
                <p className="text-sm text-gray-600">Allocated Dates: <span className="font-medium text-gray-800">{user.allocated_dates}</span></p>
              </div>
            ...
            <div className='col-span-4 w-full bg-gray-100 p-2  '> 
              {
                selectedUserId === user.user_id && user.log && user.log.map((log, index) => (
...

                          {
                            ( userInfo?.role === 'admin' ||  userInfo?.role === 'contractor' ) && (
                              <>
                                ...
                                <svg onClick={()=>deleteUserLogs(log.id)} xmlns="http://www.w3.org/2000/svg" height="22px" viewBox="0 -960 960 960" width="22px" fill="#000000"><path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z"/></svg>
                              </>
                            )
                          }
                      ...
                      <div className="flex space-x-4 text-sm text-gray-600 mt-2 justify-between">
                        ...
                        <p> Work Quality:  
                          <span className={`font-medium ${
                            log.work_quality === 'Excellent' ? 'text-green-500' :
                            log.work_quality === 'Good' ? 'text-blue-500' :
                            log.work_quality === 'Average' ? 'text-yellow-500' :
                            log.work_quality === 'Poor' ? 'text-red-500' :
                            'text-gray-500'
                          }`}>
                            {log.work_quality? log.work_quality:'N/A'}
                          </span>
                        </p>

...
export default function EquipmentGeneralDetails(props) {
    ...
    const equipment = async()=>{
        try{
            const response = await axios.get(`http://127.0.0.1:8000/api/equipment/${props.equipmentId}`,{
            headers: 
        ...
        <div className="w-full  max-w-7xl mx-auto px-6 md:px-8  ">
            ...
            <div className="flex items-center justify-center sm:justify-start  -mt-16 z-10 mb-5">
                { equipmentDetails.image ? 
                    <img src={`http://127.0.0.1:8000/storage/${equipmentDetails.image}` } alt="user-avatar-image" className="border-4 border-solid border-white rounded-full h-36 w-36 "/> :
            ...
            <div className="flex flex-col sm:flex-row max-sm:gap-5 items-center justify-between mb-1 ">
                <div className="block">
                    <h3 className="font-manrope font-bold text-4xl text-gray-900  ">  {equipmentDetails.name}   </h3>
                    <p className="font-normal text-base leading-7 text-gray-500 text-center "> {equipmentDetails.serial_number} </p>
                ...
                <button
                    ...
                    <span className="px-2 font-medium text-base leading-7 text-gray-700 transition-all duration-500 group-hover:text-indigo-600">   {equipmentDetails.category} </span>
                </button>
            </div>
            ...
            <div className="flex flex-col lg:flex-row max-lg:gap-5 items-center justify-between py-0.5">
                ...
                <div className="flex flex-col md:flex-row items-center gap-6 ">
                    ...
                    <ul className="flex items-center max-sm:justify-center max-sm:flex-wrap gap-2.5">
                        ...
                        <button onClick={()=>handleEditEquipment(equipmentDetails.id)} className="py-3.5 px-5 rounded-full bg-indigo-600 text-white font-semibold text-base leading-7 shadow-sm shadow-transparent transition-all duration-500 hover:shadow-gray-100 hover:bg-indigo-700">Edit
                        Profile</button>
        ...
        <div className=' grid md:grid-cols-2 p-2 sm:grid-cols-1 gap-2    '>
            ...
            <div className='col-span-1 grid lg:grid-cols-2 md:grid-cols-1 bg-white rounded-lg py-3 '> 
                ...
                <div className='col-span-1 items-center sm:text-center py-2 rounded-lg px-1 mb-2'>
                    <label className="text-center text-sm font-bold block">Serial Number</label> 
                    <label className="text-center text-sm px-1 block"> {equipmentDetails.serial_number}</label>
                </div>
                ...
                <div className='col-span-1 text-center py-2 rounded-lg px-1 mb-2'>
                    ...
                    <label className="text-center text-sm px-1 block"> {equipmentDetails.name}</label>
                </div>

                <div className='col-span-1 text-center py-2 rounded-lg px-1 mb-2'>
                    <label className="text-center text-sm font-bold block">Category:</label> 
                    <label className="text-center text-sm px-1 block"> {equipmentDetails.category}</label>
                </div>

                <div className='col-span-1 text-center py-2 rounded-lg px-1 mb-2'>
                    <label className="text-center text-sm font-bold block">Condition Level:</label> 
                    <label className="text-center text-sm px-1 block"> {equipmentDetails.condition_level}</label>
                </div>
            ...
            <div className='col-span-1 grid lg:grid-cols-2 md:grid-cols-1 bg-white rounded-lg '> 
                ...
                <div className='col-span-1 text-center py-2 rounded-lg px-1 mb-2'>
                    <label className="text-center text-sm font-bold block">Model:</label> 
                    <label className="text-center text-sm px-1 block"> {equipmentDetails.model}</label>
                </div>

                <div className='col-span-1 items-center sm:text-center py-2 rounded-lg px-1 mb-2'>
                    <label className="text-center text-sm font-bold block"> Status</label> 
                    <label className="text-center text-sm px-1 block"> {equipmentDetails.status}</label>
                </div> 
                

                <div className='col-span-1 text-center py-2 rounded-lg px-1 mb-2'>
                    <label className="text-center text-sm font-bold block">Purchase Price:</label> 
                    <label className="text-center text-sm px-1 block"> {equipmentDetails.purchase_price}</label>
                </div>

                <div className='col-span-1 text-center py-2 rounded-lg px-1 mb-2'>
                    <label className="text-center text-sm font-bold block">Purchase Date:</label> 
                    <label className="text-center text-sm px-1 block">{equipmentDetails.purchase_date ? equipmentDetails.purchase_date.substring(0,10): null}</label>
                </div>

...
export default function Login() {
  ...
  const login = async(e) =>{
    ...
    try{
      ...
      else if(response.data.user.role === 'labor'){
        Navigate('/my-tasks')
      }
      else{
        Navigate('/projects')
      ...
      <div className=' flex flex-col md:flex-row p-6 md:shadow-[0_3px_10px_rgb(0,0,0,0.4)] rounded-lg dark:bg-gray-800'>
        ...
        <div className='  '>
          ...
          <form onSubmit={login} className=' w-full min-w-[350px]  ' >
            ...
            <span className='flex justify-center'> <Link to="/frogot-password" className='text-center mt-4 text-[1rem]'> Forgot password!</Link> </span>
